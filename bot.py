...
BLOOD_MARK_STORIES = ["<i>{name} сжимает револьвер. На руке вспыхивает багровая метка. Глаза стекленеют. Он целится наверняка.</i>", "<i>Кровавая метка {name} загорается в темноте. Пуля найдёт свою жертву.</i>"]
REINCARNATION_STORIES = ["<i>{name} падает... но вдруг его глаза открываются. Из груди вырывается хрип. Смерть отступила. Реинкарнация сработала.</i>"]
SILENCER_STORIES = ["<i>{name} нажимает на курок. Тишина. Никто не понял, что произошло. Глушитель сделал своё дело.</i>"]
FAKE_BULLET_STORIES = ["<i>{name} нажимает на курок. БАХ! Все вздрагивают... но пули нет. Это была фальшивка. {name} усмехается.</i>"]
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS black_market_purchases (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, item_name TEXT, purchased_at TEXT, used INTEGER DEFAULT 0)''')
    
    # Добавление колонок для старых таблиц (если нет)
    try:
        c.execute("ALTER TABLE chat_settings ADD COLUMN drunk_shooter INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE chat_settings ADD COLUMN drunk_shooter_chance INTEGER DEFAULT 15")
    except: pass
    try:
        c.execute("ALTER TABLE chat_settings ADD COLUMN hardcore_allowed INTEGER DEFAULT 1")
    except: pass
    try:
        c.execute("ALTER TABLE chat_settings ADD COLUMN shields_allowed INTEGER DEFAULT 1")
    except: pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN blood_mark INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN reincarnation INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN silencer INTEGER DEFAULT 0")
    except: pass
    try:
        c.execute("ALTER TABLE users ADD COLUMN fake_bullet INTEGER DEFAULT 0")
    except: pass
    
    # Добавление товаров в чёрный рынок (если пусто)
    c.execute("SELECT COUNT(*) FROM black_market")
    if c.fetchone()[0] == 0:
        items = [
            ("Бронежилет", "shield", 300, "обычный", "shield", 2, 1),
            ...
            ("Крадущийся взгляд", "peek", 250, "редкий", "peek", 1, 1),
            ("Кровавая метка", "blood_mark", 400, "эпический", "blood_mark", 1, 1),
            ("Маска смерти", "death_mask", 600, "легендарный", "death_mask", 1, 1),
        ]
        for item in items:
...
def get_reports(status=None):
    ...
    c = conn.cursor()
    if status:
        c.execute("SELECT id, user_id, report_type, message, status, created_at, chat_id, reported_user_id FROM reports WHERE status = ? ORDER BY created_at DESC", (status,))
    else:
        c.execute("SELECT id, user_id, report_type, message, status, created_at, chat_id, reported_user_id FROM reports ORDER BY created_at DESC")
    r = c.fetchall()
    conn.close()
    return r
...
def use_promo(user_id, code, chat_id=0):
    conn = sqlite3.connect("roulette.db")
    c = conn.cursor()
    ...
    if pcid != 0 and pcid != chat_id: conn.close(); return False, "❌ Этот промокод не для этого чата!"
    ...
    u = get_user(user_id)
    result_msg = "✅ Промокод активирован!\n\n"
    
    if is_rand:
        import random
        prizes = [
            ("gc", 100), ("gc", 150), ("gc", 200), ("gc", 250), ("gc", 300),
            ("shield", 1), ("double", 1), ("insurance", 1), ("diamond_shield", 1),
            ("vip", 1), ("vip", 3), ("blood_mark", 1)
        ]
        rt2, ra2 = random.choice(prizes)
        if rt2 == "gc": update_user(user_id, gc=u["gc"]+ra2); result_msg += f"🎁 +{ra2} GC"
        elif rt2 == "shield": update_user(user_id, shields=u["shields"]+ra2); result_msg += f"🛡️ +{ra2} Щит"
        elif rt2 == "double": update_user(user_id, double_chance=u["double_chance"]+ra2); result_msg += f"⚡ +{ra2} Двойной шанс"
        elif rt2 == "insurance": update_user(user_id, insurance=u["insurance"]+ra2); result_msg += f"💰 +{ra2} Страховка"
        elif rt2 == "diamond_shield": update_user(user_id, diamond_shield=u["diamond_shield"]+ra2); result_msg += f"💎 +{ra2} Алмазный щит"
        elif rt2 == "vip": update_user(user_id, vip_level=ra2, vip_until=(datetime.now()+timedelta(days=ra2)).isoformat()); result_msg += f"👑 VIP на {ra2} дня"
        elif rt2 == "blood_mark": update_user(user_id, blood_mark=u["blood_mark"]+ra2); result_msg += f"🔪 +{ra2} Кровавая метка"
    elif disc > 0:
        update_user(user_id, discount=disc)
        result_msg += f"🏷️ Скидка {disc}% на следующую покупку в магазине!"
    elif rt == "all":
        # Комбо-промокод
        amounts = [int(x) for x in ra.split(',')]
        update_user(user_id, gc=u["gc"]+amounts[0])
        update_user(user_id, shields=u["shields"]+amounts[1])
        update_user(user_id, double_chance=u["double_chance"]+amounts[2])
        update_user(user_id, insurance=u["insurance"]+amounts[3])
        update_user(user_id, diamond_shield=u["diamond_shield"]+amounts[4])
        if len(amounts) > 5:
            update_user(user_id, vip_level=amounts[5], vip_until=(datetime.now()+timedelta(days=amounts[5])).isoformat())
        result_msg += f"💰 +{amounts[0]} GC\n🛡️ +{amounts[1]} Щит\n⚡ +{amounts[2]} Двойной шанс\n💰 +{amounts[3]} Страховка\n💎 +{amounts[4]} Алмазный щит"
        if len(amounts) > 5: result_msg += f"\n👑 VIP на {amounts[5]} дней"
    elif rt == "gc":
        update_user(user_id, gc=u["gc"]+int(ra))
        result_msg += f"💰 +{ra} GC"
    elif rt == "shield":
        update_user(user_id, shields=u["shields"]+int(ra))
        result_msg += f"🛡️ +{ra} Щит"
    elif rt == "double":
        update_user(user_id, double_chance=u["double_chance"]+int(ra))
        result_msg += f"⚡ +{ra} Двойной шанс"
    elif rt == "insurance":
        update_user(user_id, insurance=u["insurance"]+int(ra))
        result_msg += f"💰 +{ra} Страховка"
    elif rt == "diamond_shield":
        update_user(user_id, diamond_shield=u["diamond_shield"]+int(ra))
        result_msg += f"💎 +{ra} Алмазный щит"
    elif rt == "vip":
        update_user(user_id, vip_level=int(ra), vip_until=(datetime.now()+timedelta(days=int(ra))).isoformat())
        result_msg += f"👑 VIP на {ra} дней"
    elif rt == "blood_mark":
        update_user(user_id, blood_mark=u["blood_mark"]+int(ra))
        result_msg += f"🔪 +{ra} Кровавая метка"
    elif rt == "reincarnation":
        update_user(user_id, reincarnation=u["reincarnation"]+int(ra))
        result_msg += f"🔄 +{ra} Реинкарнация"
    elif rt == "silencer":
        update_user(user_id, silencer=u["silencer"]+int(ra))
        result_msg += f"🔇 +{ra} Глушитель"
    elif rt == "fake_bullet":
        update_user(user_id, fake_bullet=u["fake_bullet"]+int(ra))
        result_msg += f"💀 +{ra} Фальшивая пуля"
...
black_market_end_time = 0
black_market_items = []

def start_black_market():
    global black_market_active, black_market_end_time, black_market_items
    black_market_active = True
    black_market_end_time = time.time() + 15 * 60  # 15 минут
    ...
    # Уведомляем админа
    try:
        bot.send_message(ADMIN_ID, f"🌑 Чёрный рынок открыт!\nДоступно {len(black_market_items)} товаров на 15 минут.")
    except: pass
...
def purchase_black_market_item(user_id, item_index):
    ...
    if item_type == "shield":
        ...
        update_user(user_id, peek=u.get("peek", 0) + effect_value)
    elif item_type == "death_mask":
        update_user(user_id, death_mask=u.get("death_mask", 0) + effect_value)
    ...
    return True, f"✅ Куплено: {item_name} за {price} GC!\nРедкость: {rarity}"
...
def shop_kb(page):
    ...
    items = {
        1:[("🛡️ Щит","buy_shield",100),("💎 Алмазный щит","buy_diamond_shield",400),("🔪 Кровавая метка","buy_blood_mark",400)],
        2:[("⚡ Двойной шанс","buy_double",150),("🔄 Реинкарнация","buy_reincarnation",500),("🎯 Мастер","buy_master",250)],
        ...
        5:[("🔇 Глушитель","buy_silencer",150),("💀 Фальшивая пуля","buy_fake_bullet",100)]
    }
...
def handle_callback(call):
    ...
    if call.data == "donate_menu":
        bot.edit_message_text("<b>💳 ПОДДЕРЖАТЬ ПРОЕКТ</b>\n\n<b>Курс:</b> 10 ₽ = 350 GC\n\n<b>Способы поддержки:</b>\n\n1️⃣ <b>DonationAlerts</b>\nhttps://www.donationalerts.com/r/FxHoFiLiOn\n\n2️⃣ <b>Карта</b>\n<code>2202 2081 8206 1235</code>\n\n<b>После перевода:</b>\n• Отправьте скриншот чека @HoFiLiOnclkc\n• Укажите свой ID\n• Получите GC на баланс!", cid, mid, reply_markup=donate_menu_kb())
        bot.answer_callback_query(call.id)
        return
    
    if call.data == "donate_card":
        bot.edit_message_text("💳 <b>КАРТА</b>\n\n<code>2202 2081 8206 1235</code>\n\nПосле перевода отправьте чек @HoFiLiOnclkc", cid, mid, reply_markup=donate_menu_kb())
        bot.answer_callback_query(call.id)
        return
    ...
    buy_items = {
        ...
        "buy_blood_mark": ("blood_mark", 400, "Кровавая метка"),
        "buy_reincarnation": ("reincarnation", 500, "Реинкарнация"),
        "buy_silencer": ("silencer", 150, "Глушитель"),
        "buy_fake_bullet": ("fake_bullet", 100, "Фальшивая пуля")
    }
    ...
    if call.data == "buy_lucky":
        if u["gc"] >= 90:
            ...
            pr = random.choice([20,30,50,80,100,150,200,"shield","double","insurance","blood_mark"])
            if pr == "shield":
    ...
    if call.data == "promo_menu":
        bot.edit_message_text("<b>🎫 ПРОМОКОДЫ</b>\n\nВведи /promo КОД", cid, mid, reply_markup=promo_menu_kb())
        bot.answer_callback_query(call.id)
        return
    ...
    if call.data in ["report_player", "report_bug", "report_finance", "report_other"]:
        types = {
            "report_player": "на игрока",
            "report_bug": "баг",
            "report_finance": "финансы",
            "report_other": "другое"
        }
    ...
    if call.data == "admin_panel":
        if uid != ADMIN_ID:
            ...
            return
        bot.edit_message_text("<b>👑 АДМИН ПАНЕЛЬ</b>", cid, mid, reply_markup=admin_panel_kb())
        bot.answer_callback_query(call.id)
        return
    ...
    if call.data == "admin_black_market":
        ...
        kb.add(InlineKeyboardButton("🔙 Назад", callback_data="admin_panel"))
        bot.edit_message_text("<b>🌑 УПРАВЛЕНИЕ ЧЁРНЫМ РЫНКОМ</b>\n\nТекущее состояние: " + ("🟢 ОТКРЫТ" if black_market_active else "🔴 ЗАКРЫТ"), cid, mid, reply_markup=kb)
        bot.answer_callback_query(call.id)
        return
    ...
    if call.data == "admin_create_promo":
        if uid != ADMIN_ID: return
        bot.send_message(uid, "Введите: КОД ТИП КОЛ-ВО ЛИМИТ [ДНИ]\nТипы: gc,shield,double,insurance,diamond_shield,vip,blood_mark,reincarnation,silencer,fake_bullet,all")
    ...
    if call.data == "admin_add_gc":
        ...
        bot.register_next_step_handler_by_chat_id(uid, lambda m: add_gc_handler(m, cid, mid))
        bot.answer_callback_query(call.id)
        return
    ...
    if call.data == "admin_remove_gc":
        ...
        bot.register_next_step_handler_by_chat_id(uid, lambda m: remove_gc_handler(m, cid, mid))
        bot.answer_callback_query(call.id)
        return
    ...
    if call.data == "admin_ranks_reset":
        ...
        for r in dr:
            c.execute("INSERT INTO rank_settings VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", r)
        conn.commit()
        conn.close()
    ...
    if call.data == "black_market":
        if not black_market_active:
            bot.answer_callback_query(call.id, "🌑 Чёрный рынок закрыт! Загляни позже.", show_alert=True)
            return
        items = get_black_market_items()
        if not items:
            bot.edit_message_text("🌑 На Чёрном рынке сейчас пусто... Загляни позже.", cid, mid, reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Назад", callback_data="back")))
        else:
            ...
            for i, item in enumerate(items):
                ...
                text += f"   <i>{effect_value} x {item_type}</i>\n\n"
    ...
    if call.data == "game_history":
        # Здесь можно добавить историю игр из БД
    ...
    if call.data.startswith("game_shoot_"):
        ...
        # Применяем защиты только в аркадном режиме и если не пьяный выстрел
        ...
        # Кровавая метка
        if not drunk_shot and u2["blood_mark"] > 0 and is_dead:
            # Если есть кровавая метка — можно не использовать, но она даёт гарантию убийства
            pass
        
        if drunk_shot:
            # Пьяный выстрел в другого игрока
            victim = drunk_target
...
def report_text_handler(m, report_type, ocid, omid):
    report_type_clean = report_type.replace("report_", "")
    types = {"player": "на игрока", "bug": "баг", "finance": "финансы", "other": "другое"}
    add_report(m.from_user.id, types.get(report_type_clean, "другое"), m.text)
...
if __name__ == "__main__":
    ...
    # Запускаем планировщик для Чёрного рынка (раз в 24 часа)
    def schedule_black_market():
        while True:
            now = datetime.now()
            # Запуск в 21:00 каждый день
            ...
            start_black_market()
            time.sleep(15 * 60)  # 15 минут рынок открыт
            end_black_market()
