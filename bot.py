...
def get_shop_keyboard():
    ...
    btn1 = types.InlineKeyboardButton("🔫 Купить пулю (5⭐)", url="https://t.me/glozorgbot")
    btn2 = types.InlineKeyboardButton("🛡️ Купить броню (10⭐)", url="https://t.me/glozorgbot")
    btn3 = types.InlineKeyboardButton("🔄 Купить перекрут (3⭐)", url="https://t.me/glozorgbot")
    btn4 = types.InlineKeyboardButton("👀 Купить сканер (7⭐)", url="https://t.me/glozorgbot")
    btn5 = types.InlineKeyboardButton("💊 Купить аптечку (15⭐)", url="https://t.me/glozorgbot")
    markup.add(btn1, btn2, btn3, btn4, btn5)
    return markup

def get_game_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=1)
    btn = types.InlineKeyboardButton("🔫 Выстрелить", url="https://t.me/glozorgbot")
    markup.add(btn)
    return markup
...
Добро пожаловать, {users[user_id]['username']}!

💰 Твои монеты: {users[user_id]['coins']}
⭐ Твои Stars: {users[user_id]['stars']}
...
Нажми кнопку ниже чтобы выстрелить:
    """
    
    bot.send_message(int(lobby["chat_id"]), text, reply_markup=get_game_keyboard())
    ...
    if lobby["position"] == lobby["chamber"]:
        ...
        if len(lobby["alive"]) == 1:
            ...
            if winner_id in users:
                ...
                users[winner_id]["coins"] += 50  # +50 за победу
            for player_id in lobby["players"]:
                if player_id in users and player_id != winner_id:
                    users[player_id]["games"] += 1
                    users[player_id]["coins"] += 20  # +20 за участие
            save_json(USERS_FILE, users)
...
Нажми на нужный товар чтобы купить:
    """
...
def confirm_command(message):
    bot.reply_to(message, "✅ Функция подтверждения платежей временно не работает. Предметы будут выдаваться вручную.")
